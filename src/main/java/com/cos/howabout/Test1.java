...
// ResponseBody - MessageConvert를 타게되어있다.
// // (ViewResolver를 타지않는다.)
...
// User userEntity = userRepository.mLogin(user.getUsername(),
// user.getPassword());
// if (userEntity == null) {
...
// return "redirect:/login"; // redirect는데이터를 가지고 그담페이지로 이동한다. 데이터를 가지고
// login페이지로 이동한다는 뜻
// }
...
// model.addAttribute("postEntity", postEntity); // 게시글을 쓰는 1명이기때문에 1명걸 조인하는게
// 좋다.
...
// @PutMapping("post/{id}")
// public @ResponseBody String updatePost(@PathVariable int id, @RequestBody
// Post post) {
// Post postEntity = postRepository.findById(id).get();
